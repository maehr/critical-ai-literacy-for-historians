---
lang: de
title: Wissenschaftliches Schreiben
subtitle: Rollenbasiertes Übungsset entlang des Schreibprozesses
description: Eine Übung, die den Einsatz generativer KI als Schreibassistent in mehreren
  Rollen trainiert – mit Fokus auf Nachvollziehbarkeit, Argumentationslogik und sprachliche
  Präzision.
author:
- name: Moritz Mähr
  orcid: 0000-0002-1367-1618
  email: moritz.maehr@gmail.com
  affiliations:
  - Universität Bern
date: '2025-12-24'
date-modified: "2026-08-16"
categories: [Methoden]
image: ../../assets/images/writing_Gemini_Generated_Image_ssnsi1ssnsi1ssns.png
tags:
  - Schreibpraxis
  - Argumentation
  - Strukturierung
  - Nachvollziehbarkeit
  - Wissenschaftliche Integrität
  - KI-gestützt
difficulty: Beginner
time_estimate: "60 min"
draft: false
references:
- id: history_toolkit_en_2016
  type: book
  title: 'History: A Toolkit for Students'
  author:
  - family: Stüdli
    given: Beat
  - family: Hitz
    given: Benjamin
  translator:
  - family: Grogan
    given: Patrick
  issued:
    year: 2016
  publisher: Department of History, University of Basel
  note: English translation of 'Werkzeugkasten Geschichte'; revised online version
    of the 2010 study guide
  URL: https://dg.philhist.unibas.ch/fileadmin/user_upload/dg/Studium/Fuer_Studierende/Werkzeugkasten/Dokumente/Texte_des_Werkzeugkastens/History_a_Toolkit_en.pdf
  abstract: Study guide covering research questions, literature and source work, analytical
    techniques, academic writing, citation, and digital resources.
- id: kompass_geschichtsstudium_2025
  type: book
  title: Kompass Geschichtsstudium
  subtitle: Version FS 2026
  author:
  - family: Luther
    given: Johannes
  - family: Meier
    given: Marietta
  - family: Kaiser
    given: Rafael
  - family: Tinner
    given: Tamara Ann
  issued:
    year: 2026
  publisher: Historisches Seminar, Universität Zürich
  note: Offizielle Studienanleitung; aktualisiert am 26. Januar 2026
  URL: https://www.hist.uzh.ch/dam/jcr:363b3b70-63f1-40aa-9020-ca92e3f017ed/20251218_Kompass_V12-FS2026.pdf
  abstract: Systematic overview of study requirements and methods in history, including
    digital competence and citation of AI-based tools.
- id: leitfaden_bremen_2025
  type: book
  title: Leitfaden Geschichtswissenschaft
  subtitle: Studienleitfaden des Instituts für Geschichtswissenschaft, 8. Auflage
  issued:
    year: 2025
  publisher: Institut für Geschichtswissenschaft, Universität Bremen
  note: Überarbeitete und erweiterte 8. Auflage; enthält Hinweise zum Einsatz generativer
    KI
  URL: https://www.uni-bremen.de/fileadmin/user_upload/fachbereiche/fb8/ifges/Geschaeftsfuehrung/Sonstiges/Leitfaden_8._Auflage_IS.pdf
  abstract: Study guide covering historical theory, source criticism, academic writing
    formats, citation practices, and the use of generative AI.
- id: studienleitfaden_hamburg_2021
  type: book
  title: Studienleitfaden Fachbereich Geschichte
  author:
  - family: Lösse
    given: Esther
  - family: Anders
    given: Klaas
  - family: Szadziewski
    given: Max
  issued:
    year: 2021
  publisher: Fachbereich Geschichte, Universität Hamburg
  note: Redaktionell betreut von Monica Rüthers; abgeschlossen am 6. Januar 2021
  URL: https://www.geschichte.uni-hamburg.de/1ressourcen/studienleitfaden.pdf
  abstract: Orientation guide for history students covering organization of studies,
    research, reading, citation, and digital competence.
- id: werkszeugkasten_basel_2016
  type: book
  title: Werkzeugkasten Geschichte
  author:
  - family: Stüdli
    given: Beat
  - family: Hitz
    given: Benjamin
  issued:
    year: 2016
  publisher: Departement Geschichte, Universität Basel
  note: Revidierte Online-Version des Studienleitfadens von 2010
  URL: https://dg.philhist.unibas.ch/fileadmin/user_upload/dg/Studium/Fuer_Studierende/Werkzeugkasten/Dokumente/Texte_des_Werkzeugkastens/Werkzeugkasten_Geschichte.pdf
  abstract: Comprehensive guide to historical research practice with emphasis on source
    criticism and digital resources.
---

## Überblick und didaktisches Ziel

Diese Übung trainiert den **kritischen Einsatz generativer KI als Schreibassistent** in mehreren klar abgegrenzten Rollen. Die Rollen sind so angeordnet, dass sie einem typischen wissenschaftlichen Schreibprozess entsprechen: **Problem klären → Ideen entwickeln → strukturieren → entwerfen → revidieren → finalisieren**.

Ziel ist nicht, Textproduktion zu «automatisieren», sondern **Entscheidungsqualität** zu erhöhen: bessere Fragestellungen, konsistentere Argumente, präzisere Sprache – bei gleichzeitiger **Nachvollziehbarkeit** der Autor:innenschaft und der verwendeten Evidenz.

## Auf einen Blick

| Angabe                   | Wert                                                                                           |
| ------------------------ | ---------------------------------------------------------------------------------------------- |
| **Dauer**                | ca. 45 bis 60 Minuten                                                                          |
| **Niveau**               | Beginner                                                                                       |
| **Kernpfad**             | 1 → 2 → 3 → 4 → 6 → 8                                                                          |
| **Optionale Vertiefung** | 5 (Blockaden), 7 (Stil und Übersetzung), 9 (Lesbarkeitstest)                                   |
| **Abgabe (Minimum)**     | Ein überarbeiteter Textabschnitt. [KI-Protokoll](../glossary.qmd#gl-ki-protokoll): 6 Einträge. |

Der Kernpfad führt Sie in 45 bis 60 Minuten zu einem überarbeiteten Text. Jede optionale Vertiefung kostet ca. 15 Minuten zusätzlich. Führen Sie pro Schritt des Kernpfads einen Eintrag im KI-Protokoll.

## Was Sie brauchen

**Fachliches Wissen**

- Grundverständnis historischer Forschungsmethoden
- Basiskenntnisse im Umgang mit generativer KI, insbesondere Prompting

::: callout-note

## Prompt Engineering

Falls Sie noch nicht mit Prompting vertraut sind, empfehlen wir die Übung [Prompt Engineering](prompt-engineering.qmd) zuerst zu durchlaufen.

:::

**Fähigkeiten des KI-Systems**

Das System muss die zwei folgenden Dinge können:

- Nur mit dem Material arbeiten, das Sie ihm geben. Sie prüfen diese Vorgabe in jedem Schritt.
- Eine Antwort in einem Format liefern, das Sie vorgeben, zum Beispiel als Gliederung.

Diese Übung braucht weder eine Websuche noch einen Datei-Upload. Ein kostenloses Konto genügt.

**Weitere Werkzeuge und Zugänge**

- Ein eigenes Themenfragment, zum Beispiel aus einer Seminararbeit. Ein Textabschnitt von einer halben bis einer Seite genügt.
- Eigene Stichpunkte oder Notizen zum Thema, mindestens 10 Punkte.
- Eine Datei für Ihr KI-Protokoll.

{{< include ../_ki-anforderungen.qmd >}}

## Lernziele

- Antworten als Hypothesen/Entwürfe behandeln und systematisch prüfen.
- Prompts so gestalten, dass KI nicht 'erfindet', sondern mit bereitgestelltem Material arbeitet.
- Eine Argumentationsstruktur (These–Belege–Schluss) entwerfen und auf Konsistenz testen.
- Absätze aus eigenen Stichpunkten generieren und anschliessend fachlich/terminologisch revidieren.
- Die Revision in zwei Schritten durchführen: zuerst Logik und Verständlichkeit, dann Sprache und Stil.
- Den eigenen KI-Einsatz transparent dokumentieren (KI-Protokoll).

## Methodischer Rahmen: KI als Schreibinfrastruktur

In dieser Übung wird KI als **heuristisches Schreibwerkzeug** verstanden: Sie unterstützt bei _Formulierungen, Strukturierung und Prüfung_, ersetzt aber weder **Quellenarbeit** noch **interpretative Entscheidungen**. [@bender2021stochastic; @campbell2025historian; @oberbichler2025implementing] Methodisch hilfreich ist eine klare Arbeitsteilung:

- **Geschichtswissenschaftliche Autor:innenschaft:** Fragestellung, Evidenzwahl, Interpretation, normative Setzungen.
- **KI-Beitrag (instrumentell):** Varianten generieren, Strukturvorschläge, Lesbarkeitstests, Fehlerdiagnose.
- **Prüfverfahren (obligatorisch):** Jede KI-Aussage muss entweder (a) auf Ihr Input-Material rückführbar oder (b) als Hypothese gekennzeichnet und extern verifiziert sein. [@turingway2022handbook]

Für verlässlicheres Arbeiten nutzen Sie konsequent zwei Prompt-Prinzipien: 1. **Eingrenzung des Arbeitsraums** («arbeite nur mit meinen Stichpunkten; markiere Unsicherheiten»). 2. **Formatierte Antworten** (Listen/Argumentmaps), damit Sie systematisch prüfen und selektiv übernehmen können.

::: callout-important

## Grundregel: KI darf nichts «erfinden»

Nutzen Sie KI nur als **Transformations- und Prüfwerkzeug** für _Ihr_ Material (Notizen, Quellenexzerpte, Literaturzusammenfassungen, Gliederungsentwürfe). Wenn KI zusätzliche Fakten, Literatur oder Quellen nennt, gilt das als **Recherche-Hypothese**, die Sie **extern verifizieren** müssen.
:::

{{< include ../_ki-protokoll.qmd >}}

## Struktur der Übung

Die Rollen sind zu **Phasen** gebündelt. Sie können einzelne Schritte überspringen. Der Kernpfad im Abschnitt «Auf einen Blick» führt Sie durch eine vollständige Runde bis zur Revision.

1. **Verständnischecker** (Auftrag klären, blinde Flecken identifizieren)\
2. **Brainstorming-Partner + Devil’s Advocate** (Ideenraum öffnen + Gegenargumente)\
3. **Strukturierer:in** (Argumentationsarchitektur)\
4. **Text aus Stichpunkten** (Drafting unter Faktendisziplin)\
5. **Blockaden-Löser** (lokale Schreibprobleme/Übergänge)\
6. **Logik-Prüfer** (Kohärenz, Belege, Schlussregeln)\
7. **Korrektorat + Übersetzer** (Stil/Grammatik/Register; optional Mehrsprachigkeit)\
8. **Abstract-Schreiber** (Kompression, Fokus, «So what?»)\
9. **Verständnischecker (Re-Run)** (Lesbarkeit aus Zielpublikumsperspektive)

## 1. Verständnischecker

### Ziel

Den Schreibauftrag und die eigene Argumentationsabsicht so präzise machen, dass spätere KI-Schritte nicht «ausweichen» oder falsche Prämissen stabilisieren.

### Aufgabe

1. Geben Sie KI **nur** Folgendes: Thema in 2–3 Sätzen, Kontext (Kurs/Genre), Zielpublikum, Umfang, Abgabeformat.\
2. Lassen Sie KI **Rückfragen** stellen und Unsicherheiten markieren.

**Prompt-Gerüst**

```markdown
Du bist Verständnischecker. Stelle mir 8–12 präzise Rückfragen, um Schreibauftrag und Argumentationsziel zu klären.
Markiere:
(1) fehlende Informationen,
(2) implizite Annahmen,
(3) Begriffe, die operativ definiert werden müssen.
Keine Vorschläge für Inhalte, nur Fragen und Diagnose.

Kontext: …
Thema/These (vorläufig): …
Genre/Umfang: …
Zielpublikum: …
```

### Kritischer Abgleich

- Welche Rückfragen zwingen Sie zu **Begriffsklärungen** (z.B. «Modernisierung», «Neutralität», «Governance»)?\
- Welche Fragen würden Sie **anders** priorisieren – und warum?

**Ergebnis:**

- 1 revidierter Auftrag (3–5 Sätze)
- 3 definierte Kernbegriffe

## 2. Brainstorming-Partner + Devil’s Advocate

### Ziel

Ideen generieren, ohne Evidenz zu erfinden, und sofort Gegenargumente/Alternativdeutungen mitdenken.

### Aufgabe A: Brainstorming (ideenorientiert, evidenzneutral)

```markdown
Rolle: Brainstorming-Partner. Generiere 12 mögliche Argumentlinien/Hypothesen zu meinem Thema.
Bedingungen:
- Formuliere jede Linie als überprüfbare Behauptung (keine Facts).
- Gib zu jeder Linie: welche Art von Evidenz ich bräuchte (Quellentypen/Literaturtypen).
- Vermeide konkrete Literaturtitel oder Archivsignaturen; wenn nötig, formuliere als Suchheuristik.

Mein Thema: …
Mein Material (nur Typen, keine sensiblen Inhalte): …
```

### Aufgabe B: Devil’s Advocate (konfrontativ, aber produktiv)

```markdown
Rolle: Devil’s Advocate. Wähle die 3 plausibelsten meiner Hypothesen und argumentiere jeweils dagegen.
Für jedes Gegenargument:
- Benenne die verletzliche Annahme.
- Schlage einen «Stress-Test» vor (welcher Befund würde mein Argument kippen?).
- Formuliere eine alternative Deutung (max. 2 Sätze).
Nutze keine externen Fakten.
Hypothesen: …
```

### Kritischer Abgleich

- Welche Hypothesen sind **falsifizierbar** (im Sinne klarer Widerlegungsbedingungen)?\
- Wo droht **Presentismus** (Gegenwartskategorien in die Vergangenheit projizieren)?

**Ergebnis:**

- 1 priorisierte Hypothese oder These
- 2 zentrale Gegenargumente
- 1 geplanter Stress-Test

## 3. Strukturierer:in

### Ziel

Eine überprüfbare **Argumentationskette** entwerfen (These → Unterthesen → Belege → Schluss), bevor Text generiert wird.

### Aufgabe

Setzen Sie im folgenden Prompt Ihre eigene Wortzahl ein. Das Beispiel nennt 3000 Wörter.

```markdown
Rolle: Strukturierer:in.
Erstelle eine Gliederung (max. 8 Hauptpunkte) für einen Text von ca. 3000 Wörtern.
Für jeden Punkt:
- Ziel des Abschnitts (1 Satz)
- zentrale Behauptung (1 Satz)
- benötigte Evidenz (Stichworte; «aus meinen Notizen/Quellen»)
- Übergang zum nächsten Punkt (1 Satz)

Vorgaben:
- Keine neuen Fakten.
- Wenn Evidenz fehlt: markiere als [EVIDENZLÜCKE] statt zu ergänzen.

Material (Stichpunkte/Notizen): …
```

### Qualitätscheck

- Enthält jede Behauptung eine **Evidenzspur** (mindestens als Platzhalter)?\
- Gibt es Abschnitte, die nur «Überblick» liefern, aber keine argumentative Funktion haben?

**Ergebnis:**

- 1 Gliederung
- 2–3 markierte Evidenzlücken

## 4. Text aus Stichpunkten

### Ziel

Aus eigenem Material Absätze formulieren lassen, ohne inhaltliche Kontrolle abzugeben.

### Aufgabe

Wählen Sie **einen** Gliederungspunkt (z.B. «Hauptargument 1») und liefern Sie 8–12 Bullet Points (Belege, Beobachtungen, Zitate/Paraphrasen, Begriffe).

```markdown
Rolle: Textgenerator aus Stichpunkten.
Schreibe einen Absatz (120–180 Wörter) in akademischem Deutsch.
Bedingungen:
- Verwende ausschliesslich Informationen aus meinen Stichpunkten.
- Keine neuen Beispiele, Daten, Namen, Quellen.
- Wenn ein Satz zusätzliche Information bräuchte: [UNSICHER] markieren.
- Behalte meine Terminologie; frage nach, wenn unklar.

Stichpunkte: …
```

### Kritischer Abgleich

- Streichen Sie alle Sätze, die nicht direkt auf Ihre Stichpunkte rückführbar sind.\
- Prüfen Sie Terminologie: Wo glättet KI Begriffe (z.B. «Staat», «Gesellschaft», «Akteur») unzulässig?

**Ergebnis:**

- 1 Absatz
- 1 zweite Version des Absatzes, in der Sie 3–5 Sätze eigenständig präzisiert haben

## 5. Blockaden-Löser

### Ziel

Lokale Probleme lösen (Übergänge, Einstiegssätze, Begriffsdefinitionen), ohne den Text «neu schreiben» zu lassen.

### Mikro-Übungen (wählen Sie 2)

**A) Übergang bauen**

```markdown
Rolle: Blockaden-Löser.
Formuliere 5 Varianten für einen Übergangssatz zwischen Absatz A und B.
Variante 1 neutral, 2 stärker argumentativ, 3 stärker erklärend, 4 knapper, 5 mit Kontrastmarker.
Absatz A: …
Absatz B: …
```

**B) Definition operationalisieren**

Ersetzen Sie im folgenden Prompt den Begriff «Neutralität» durch Ihren eigenen Begriff.

```markdown
Formuliere 3 mögliche Arbeitsdefinitionen für den Begriff «Neutralität» im Kontext meines Themas.
Für jede Definition:
- Abgrenzung zu 1 Nachbarbegriff
- 1 beobachtbares Indiz (woran erkenne ich «Neutralität» in Quellen?)
Keine externen Fakten.
Kontext: …
```

**C) Satzdiagnose**

```markdown
Markiere in meinem Absatz:
- den zentralen Satz (topic sentence),
- Sätze mit unklarem Referent («dies», «dabei», «man»),
- Sätze, die eigentlich zwei Behauptungen enthalten.
Schlage minimale Umstellungen vor (keine Umschreibung des Inhalts).
Text: …
```

**Ergebnis:**

- 1 überarbeiteter Übergang oder 1 überarbeitete Definition
- 1 kurze Begründung, warum Sie diese Variante gewählt haben

## 6. Logik-Prüfer

### Ziel

Argumentationslogik sichtbar machen: Stützen Belege die Behauptungen? Sind Schlussregeln nachvollziehbar? Wo fehlen Prämissen?

### Aufgabe

```markdown
Rolle: Logik-Prüfer.
Analysiere meinen Text (max. 300 Wörter) als Argumentmap:
1) Hauptthese
2) Unterthesen
3) explizite Belege (aus dem Text)
4) implizite Prämissen (die ich offenbar voraussetze)
5) mögliche logische Sprünge oder Widersprüche
Gib dann 5 konkrete Revisionsempfehlungen, priorisiert nach Wirkung.
Text: …
```

### Kritischer Abgleich

- Welche «impliziten Prämissen» akzeptieren Sie – welche müssen belegt oder umformuliert werden?\
- Wo sind Kausalbehauptungen eigentlich nur **zeitliche Abfolge**?

**Ergebnis:**

- 1 Argumentmap
- 2 gezielte Revisionen im Text, mit einer Markierung, was sich logisch verbessert

## 7. Korrektorat + Übersetzer

### Ziel

Sprache und Register verbessern, ohne die argumentative Substanz zu verändern; optional Terminologie konsistent in eine zweite Sprache übertragen.

### Aufgabe A: Korrektorat (minimal-invasiv)

```markdown
Rolle: Korrektorat.
Verbessere Grammatik, Stil und Präzision, aber ändere keine inhaltlichen Aussagen.
Arbeite minimal-invasiv:
- Ersetze vage Wörter (z.B. «wichtig», «zeigt») nur, wenn eindeutig.
- Erhalte Fachtermini; wenn unsicher: [TERM?] markieren.
Gib am Ende eine Liste der 5 häufigsten Musterfehler, die du korrigiert hast.
Text: …
```

### Aufgabe B: Übersetzen (optional)

```markdown
Rolle: Übersetzer.
Übersetze den Text ins Englische (oder Französische/Spanische).
Bedingungen:
- Bewahre Register (akademisch) und Hedge-Marker (z.B. «könnte», «legt nahe»).
- Erhalte Eigennamen und Zitate unverändert.
- Gib eine Mini-Glossar-Tabelle (5–10 Termpaare) für Schlüsselbegriffe.
Text: …
Zielsprache: …
```

**Ergebnis:**

- 1 korrigierte Version
- optional: 1 Übersetzung
- optional: 1 Glossar

## 8. Abstract-Schreiber

### Ziel

Den Text auf seine tragende Aussage verdichten: Forschungsproblem, Vorgehen, Befund/Argument, Relevanz.

### Aufgabe

```markdown
Rolle: Abstract-Schreiber.
Schreibe ein Abstract mit 120–160 Wörtern.
Muss enthalten:
- Forschungsproblem (1 Satz)
- Ansatz/Material (1 Satz)
- Kernargument/Befund (2–3 Sätze)
- Beitrag/Relevanz («so what?») (1 Satz)
Keine neuen Fakten; wenn unklar: [UNKLAR] markieren.
Textbasis: …
```

### Qualitätscheck

- Deckt das Abstract die **tatsächliche** Argumentationsspitze ab oder nur den Themenbereich?\
- Entspricht die behauptete Methode dem, was Sie wirklich tun (Quellenarbeit vs. Literaturdiskussion)?

**Ergebnis:**

- 1 Abstract
- 1 Meta-Kommentar in 2 Sätzen: Was hat die KI fälschlich betont oder ausgelassen?

## 9. Verständnischecker (Re-Run)

### Ziel

Lesbarkeit und Interpretationsspielräume aus Sicht des Zielpublikums testen.

### Aufgabe

```markdown
Rolle: Verständnischecker.
Lies meinen Text als kritische:r Leser:in im Seminar.
1) Was ist die These in einem Satz?
2) Welche 3 Punkte sind unklar oder missverständlich (jeweils mit Zitatstelle)?
3) Welche Stelle wirkt unbelegt?
4) Welche zwei Fragen würdest du mir im Feedback stellen?
Text: …
```

**Ergebnis:**

- 3 präzisierte Stellen im Text, je 1–2 Sätze überarbeitet

## Lernergebnis

Am Ende haben Sie ein kleines Paket an Artefakten, das wissenschaftlich weiterverwendbar ist:

- Gliederung (mit Evidenzlücken)\
- 1–3 überarbeitete Absätze (mit nachvollziehbaren Revisionen)\
- Argumentmap (Logikprüfung)\
- Abstract (und optional Übersetzung + Glossar)\
- KI-Protokoll mit mindestens 6 Einträgen

## Weiterführende Ressourcen

- [AI Pedagogy Project: LLM Tutorial](https://aipedagogy.org/guide/llm-tutorial/) – Grenzen, Halluzinationen, Verifikation und kritische Nutzung von LLMs.
- [The Turing Way](https://book.the-turing-way.org/) – Handbuch für reproduzierbare, ethische und kollaborative Forschung.

## Bibliographie

::: {#refs}
:::
